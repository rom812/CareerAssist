...
resource "aws_s3_bucket" "vectors" {
  bucket = "career-vectors-${data.aws_caller_identity.current.account_id}"
...
resource "aws_iam_role" "lambda_role" {
  name = "career-ingest-lambda-role"
...
resource "aws_iam_role_policy" "lambda_policy" {
  name = "career-ingest-lambda-policy"
