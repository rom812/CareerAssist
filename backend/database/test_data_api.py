...
def get_cluster_details(region):
    ...
    if cluster_arn and secret_arn:
        ...
        try:
            ...
            print("💡 Run: aws rds modify-db-cluster --db-cluster-identifier career-aurora-cluster --enable-http-endpoint --apply-immediately")
            return None, None
            else:
    ...
    try:
        # Get cluster ARN
        response = rds_client.describe_db_clusters(
            DBClusterIdentifier='career-aurora-cluster'
        )
        
        if not response['DBClusters']:
            print("❌ Aurora cluster 'career-aurora-cluster' not found")
            return None, None
        ...
        # Find the most recently created aurora secret for career
        secrets = secrets_client.list_secrets()
        aurora_secrets = []
        
        for secret in secrets['SecretList']:
            if 'aurora' in secret['Name'].lower() and 'career' in secret['Name'].lower():
...
def test_data_api(cluster_arn, secret_arn, region):
    ...
    db_name = os.getenv('AURORA_DATABASE', 'career')
    ...
    try:
        ...
        if error_code == 'BadRequestException':
            ...
            print(f"   ⚠️  Database '{db_name}' might not exist or credentials are incorrect")
            ...
            try:
                ...
                print(f"   ✅ Connection successful (but '{db_name}' database may not exist)")
                return True
            except:
                pass
        else:
    ...
    try:
        response = client.execute_statement(
            ...
            secretArn=secret_arn,
            database=db_name,
            sql=f"SELECT pg_database_size('{db_name}') as size_bytes"
        )
