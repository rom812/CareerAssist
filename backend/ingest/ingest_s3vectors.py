...
VECTOR_BUCKET = os.environ.get("VECTOR_BUCKET", "career-vectors")
SAGEMAKER_ENDPOINT = os.environ.get("SAGEMAKER_ENDPOINT")
INDEX_NAME = os.environ.get("INDEX_NAME", "career-knowledge")

# Career content types supported
CONTENT_TYPES = [
    "cv_bullet_template",
    "interview_question",
    "ats_keyword_guide",
...
def get_embedding(text):
    ...
    response = sagemaker_runtime.invoke_endpoint(
        EndpointName=SAGEMAKER_ENDPOINT, ContentType="application/json", Body=json.dumps({"inputs": text})
    )
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if not text:
            return {"statusCode": 400, "body": json.dumps({"error": "Missing required field: text"})}
        ...
        return {
            "statusCode": 200,
            "body": json.dumps({"message": "Document indexed successfully", "document_id": vector_id}),
        }
