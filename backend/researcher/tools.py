...
def _ingest(document: dict[str, Any]) -> dict[str, Any]:
    ...
    with httpx.Client() as client:
        response = client.post(CAREER_API_ENDPOINT, json=document, headers={"x-api-key": CAREER_API_KEY}, timeout=30.0)
...
def ingest_career_document(topic: str, analysis: str) -> dict[str, Any]:
    ...
    if not CAREER_API_ENDPOINT or not CAREER_API_KEY:
        return {"success": False, "error": "Career API not configured. Running in local mode."}

    document = {"text": analysis, "metadata": {"topic": topic, "timestamp": datetime.now(UTC).isoformat()}}
...
def store_discovered_job(
    ...
    if source.lower() not in VALID_JOB_SOURCES:
        return {"success": False, "error": f"Invalid source '{source}'. Must be one of: {VALID_JOB_SOURCES}"}

    # Check database configuration
    if not AURORA_CLUSTER_ARN or not AURORA_SECRET_ARN:
        print("[store_discovered_job] Database not configured, skipping DB storage")
        return {
            "success": False,
...
def store_research_finding(
    ...
    # Check database configuration
    if not AURORA_CLUSTER_ARN or not AURORA_SECRET_ARN:
        print("[store_research_finding] Database not configured, skipping DB storage")
        return {
            "success": False,
    ...
    try:
        ...
        return {"success": True, "finding_id": finding_id, "message": f"Successfully stored research finding: {title}"}
    except Exception as e:
