#!/usr/bin/env python3
"""
Watch CloudWatch logs from all CareerAssist agents in real-time.
...
COLORS = {
    ...
    "EXTRACTOR": "\033[93m",  # Yellow
    "ANALYZER": "\033[92m",  # Green
    "CHARTER": "\033[96m",  # Cyan
    "INTERVIEWER": "\033[95m",  # Magenta
    "ERROR": "\033[91m",  # Red
...
# Agent log groups
LOG_GROUPS = {
    "ORCHESTRATOR": "/aws/lambda/career-orchestrator",
    "EXTRACTOR": "/aws/lambda/career-extractor",
    "ANALYZER": "/aws/lambda/career-analyzer",
    "CHARTER": "/aws/lambda/career-charter",
    "INTERVIEWER": "/aws/lambda/career-interviewer",
}
...
class AgentLogWatcher:
            ...
            for stream in response["logStreams"]:
                ...
                try:
                    events_response = self.logs_client.filter_log_events(
                        logGroupName=log_group, logStreamNames=[stream_name], startTime=start_time, limit=100
                    )
    ...
    def watch(self, poll_interval: int = 2):
        ...
        print(f"{COLORS['BOLD']}Watching CloudWatch logs for all CareerAssist agents...{COLORS['RESET']}")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Watch CloudWatch logs from all CareerAssist agents")
