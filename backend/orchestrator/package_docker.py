#!/usr/bin/env python3
"""
Package the Orchestrator Lambda function using Docker for AWS compatibility.
"""
...
def package_lambda():
    ...
    temp_dir = str(orchestrator_dir / "build_temp")
    ...
    requirements_result = run_command(["uv", "export", "--no-hashes", "--no-emit-project"], cwd=str(orchestrator_dir))
    ...
    shutil.copy(orchestrator_dir / "lambda_handler.py", package_dir)
    shutil.copy(orchestrator_dir / "agent.py", package_dir)
    shutil.copy(orchestrator_dir / "templates.py", package_dir)
    shutil.copy(orchestrator_dir / "observability.py", package_dir)
...
def main():
    parser = argparse.ArgumentParser(description="Package Orchestrator Lambda for deployment")
