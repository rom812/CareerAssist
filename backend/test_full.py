...
def setup_test_data(db):
    ...
    if not user:
        user_data = UserCreate(clerk_user_id=test_user_id, display_name="Test User", email="testuser@example.com")
...
def main():
    ...
    sample_job = {
        ...
        "nice_to_have": [{"text": "Kubernetes knowledge", "type": "nice_to_have", "category": "technical"}],
    ...
    job_data = {
        ...
        "status": "pending",
        "input_data": {"cv_profile": sample_cv, "job_profile": sample_job},
        "request_payload": {
            "analysis_type": "full",
    ...
    response = sqs.send_message(QueueUrl=queue_url, MessageBody=json.dumps({"job_id": job_id}))
