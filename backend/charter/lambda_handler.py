...
async def run_charter_agent(job_id: str, applications_data: dict[str, Any], db=None) -> dict[str, Any]:
    ...
    with trace("Charter Agent"):
        agent = Agent(name="Career Analytics", instructions=CHARTER_INSTRUCTIONS, model=model)
        ...
        try:
            ...
            return {"success": True, "charts": charts_data.get("charts", []), "raw_response": response}
        except json.JSONDecodeError as e:
            logger.error(f"Charter: JSON parse error: {e}")
            logger.error(f"Charter: Raw response: {response[:500]}")
            return {"success": False, "error": f"Failed to parse charts JSON: {e}", "raw_response": response}
    ...
    with observe(
        ...
        try:
            ...
            if not applications_data:
                ...
                if user_id:
                    try:
                        ...
                        applications_data = {"applications": [dict(app) for app in apps] if apps else []}
            ...
            if result.get("success") and job_id != "unknown":
                try:
                    db.client.update(
                        "jobs", {"charts": json.dumps(result.get("charts", []))}, "id = :id", {"id": job_id}
                    )
                except Exception as e:
