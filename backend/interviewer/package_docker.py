#!/usr/bin/env python3
"""
Package the Interviewer Lambda function using Docker for AWS compatibility.
"""
...
def package_lambda():
    ...
    interviewer_dir = Path(__file__).parent.absolute()
    backend_dir = interviewer_dir.parent
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        requirements_result = run_command(
            ["uv", "export", "--no-hashes", "--no-emit-project"], cwd=str(interviewer_dir)
        )
        ...
        docker_cmd = [
            ...
            "-c",
            """cd /build && pip install --target ./package -r requirements.txt && pip install --target ./package --no-deps /build/database""",
        ]

        run_command(docker_cmd)

        # Copy Lambda handler, agent, templates, and observability
        shutil.copy(interviewer_dir / "lambda_handler.py", package_dir)
        shutil.copy(interviewer_dir / "agent.py", package_dir)
        shutil.copy(interviewer_dir / "templates.py", package_dir)
        shutil.copy(interviewer_dir / "observability.py", package_dir)

        # Create the zip file
        zip_path = interviewer_dir / "interviewer_lambda.zip"
...
def main():
    parser = argparse.ArgumentParser(description="Package Interviewer Lambda for deployment")
